...
{
	float diff = md->timer.fps - md->prm.cap_fps;
	float abs_diff = fabsf(diff);
	float factor = abs_diff * 0.1f;
	if (diff < -1.0f && md->prm.ray_mod < 20.0f)
		md->prm.ray_mod += fminf(2.0f, factor);
	else if (diff > 1.0f && md->prm.ray_mod > 1)
		md->prm.ray_mod -= fminf(2.0f, factor);
	if (md->prm.ray_mod < 1.4f) md->prm.ray_mod = 1.4f;
	if (md->prm.ray_mod > 10.0f) md->prm.ray_mod = 10.0f;
}
