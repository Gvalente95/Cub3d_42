...
void	setCorner(int tlen, t_vec2f off, t_ray* ray) {
	float lim = 5.0f;
	if (off.x <= lim && off.y <= lim)
		ray->corner = CORNER_TL;
	else if (off.x <= lim && (tlen - off.y) <= lim)
		ray->corner = CORNER_BL;
	else if ((tlen - off.x) <= lim && off.y <= lim)
		ray->corner = CORNER_TR;
	else if ((tlen - off.x) <= lim && (tlen - off.y) <= lim)
...
int	update_ray_grid_pos(t_md *md, t_ray *ray)
{
	...
	if (edge_dist.x <= 1 || edge_dist.y <= 1)
	{
		...
		if (fabsf(edge_dist.x - edge_dist.y) < 1.0f && ray->index > 0)
			ray->vertical_hit = md->rays[ray->index - 1].vertical_hit;
	}
	setCorner(tlen, off, ray);
	return (is_on_grid);
}
...
int	correct_fisheye(t_md *md, t_ray *ray, t_ent *e, float dist)
{
	...
	plane_proj_height = md->win_sz.y * e->size.y;
