...
int				get_row_len(t_md* md, float ray_dst);

//	raycasting/ray_cast.c
int				cast_ray(t_md *md, t_ray *ray, t_vec2 visu_offset);
void			compute_ray_directions(t_md *md, t_vec3f *dirs, int rays_len);
void			cast_rays(t_md *md);
void			paint_ent(t_md *md, t_ent *e, t_vec2 txtr_coord);
int				try_set_status(t_md *md, t_BA_d *bd, t_ent *target, t_move *mv);
...
int				translate_ray(t_md *md, t_ray *ray, t_ent *portal_data, int view_i);
...
void			init_base_ray(t_ray *ray, int i, t_vec3f start_p, float dist);
int				check_hit(t_md *md, t_ray *ray, t_ent *ent, t_ent_type type);
...
void			show_minimap_entity(t_md *md, t_ent *e, \
t_image *screen, int no_redraw);
void			render_minimap(t_md *md, t_mmap *mp);

//	render/filters.c
void			apply_antialiasing(t_image *img);
void			apply_scanlines(t_image *img, float factor);
void			apply_noise(t_md *md, t_image *img, float factr, float clr_len);

//	render/render_hud.c
void			draw_locks_icons(t_md *md, t_hud *hud, t_vec2 winsz);
void			draw_info(t_md *md, t_vec2 *screen_p, t_image *icon, int value);
...
void			draw_sprite_thread(t_md *md, t_ent *e, int had_door, float fogA);
...
int				use_held_item(t_md *md, t_inventory *inv, t_ent *targt, int i);
...
void			add_alert(t_md *md, double dur, int *out_cond, const char *msg);
void			render_held_item(t_md *md, t_inventory *inv, int hand_index);
int				contains_valid_character(char *line, const char *valid_chars);
void			update_pointed(t_md *md, t_vec2 draw_p, t_vec3 scale, t_ent *e);
...
void			set_action(t_md *md, t_move *moveToUse, int actionType, \
const char* msg, ...);
...
void			render_team_logos(t_md *md, t_BA_d *bd, int side, int team_sz);
...
int				display_options(t_md *md, t_ent *e, const char *lbl, t_txtd txtd);
...
void			draw_from_pos(t_image *src, t_image *dst, t_vec2 p, t_vec2 start);
...
void			init_pk_frames(t_md *md, t_texture_data *txd);
