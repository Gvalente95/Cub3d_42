...
/*   Updated: 2025/07/01 04:57:22 by giuliovalen      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	pxl_draw(t_md *md, t_ray_draw_d *d, t_vec2 win_sz)
{
	...
	win_p.y = d->y_start + d->win_start.y - md->cam.pos.z + \
		(d->ray->wall_hit->pos.z / d->ray->distance);
	...
	if (win_p.x < 0 || win_p.x > win_sz.x || skip_pxl(md, d))
		return (1);
	...
	return (d->ray->dirty_checks[win_p.y] = 1, 1);
}
