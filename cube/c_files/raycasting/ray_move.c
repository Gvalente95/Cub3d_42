...
/*   Updated: 2025/07/01 04:45:29 by giuliovalen      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	I need a way to check for rays from outside map that will never treach map
static int	ray_can_stop(t_md *md, t_ray *r, t_ent *hit, int tlen)
{
	...
	if (r->hits_len >= md->prm.max_view_sprite)
		return (1);
	if (hit && r->check_hit && r->check_hit == hit)
		return (1);
	if (r->check_hit && r->steps > r->init_steps + 2)
		return (1);
